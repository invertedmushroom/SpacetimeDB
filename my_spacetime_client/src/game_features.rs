...
// Import reducer traits to bring methods into scope
...
impl ChunkSubscriptionManager {
    pub fn new(conn: DbConnection) -> Self {
        ...
        let manager = Self { conn, subscription_handle: Some(init_sub), current_chunk: None };
        manager
    }
...
mod tests {
    ...
    /// Fake actions for unit testing GameActions facade
    struct FakeActions {
        state: Arc<Mutex<GameState>>,
        pub moved: Cell<bool>,
        pub picked: Cell<bool>,
    ...
    impl GameActions for FakeActions {
        fn move_player(&self, _new_x: f32, _new_y: f32) -> Result<(), GameError> {
            if self.should_fail {
                Err(GameError::NetworkError("Connection lost".into()))
            } else {
                self.moved.set(true);
                Ok(())
            }
        ...
        fn pickup_item(&self, item_id: u64) -> Result<(), GameError> {
            if self.should_fail {
                ...
                Err(GameError::NotFound("Invalid item".into()))
            } else {
        ...
        fn drop_item(&self, _item_id: u64) -> Result<(), GameError> {
            if self.should_fail {
                Err(GameError::NetworkError("Connection lost".into()))
            } else {
        ...
        fn attack_player(&self, _target: Identity, _damage: u32) -> Result<(), GameError> {
            if self.should_fail {
                Err(GameError::NetworkError("Connection lost".into()))
            } else {
                Ok(())
        ...
        fn aoe_attack(&self, _center_x: f32, _center_y: f32, _radius: f32, _damage: u32) -> Result<(), GameError> {
            if self.should_fail {
                Err(GameError::NetworkError("Connection lost".into()))
            } else {
                Ok(())
    ...
    #[test]
    fn test_move_player_ok() {
        let fake = FakeActions::new(false);
        assert!(fake.move_player(1.0, 2.0).is_ok());
        assert!(fake.moved.get());
    }

    #[test]
    fn test_pickup_item_ok() {
        let fake = FakeActions::new(false);
        assert!(fake.pickup_item(1).is_ok());
        assert!(fake.picked.get());
    }

    #[test]
    fn test_pickup_item_err() {
        let fake = FakeActions::new(false);
        let err = fake.pickup_item(0).unwrap_err();
    ...
    fn test_command_pattern() {
        let fake = FakeActions::new(false);
        let command = MoveAndPickupCommand {
            item_id: 1,
            target_pos: (10.0, 20.0),
        };
        
        assert!(execute_command(&fake, command).is_ok());
        assert!(fake.moved.get());
        assert!(fake.picked.get());
    }
    ...
    fn test_retry_mechanism() {
        let mut attempts = 0;
        let fake = FakeActions::new(false);
        let result = fake.with_retry(|| {
            attempts += 1;
            if attempts < 2 {
                Err(GameError::NetworkError("Temporary failure".into()))
            } else {
                Ok(())
            }
        }, 3);
    ...
    fn test_game_state() {
        let fake = FakeActions::new(false);
        let state = fake.get_state();
        
        // Update state
        {
            let mut state_guard = state.lock().unwrap();
            let player = Player {
                player_id: Identity::default(),
                username: "TestPlayer".to_string(),
                position_x: 10.0,
                position_y: 20.0,
                chunk_x: 0,  // Adding missing chunk_x field
                chunk_y: 0,  // Adding missing chunk_y field
                health: 100,
                score: 0,
                status: crate::module_bindings::PlayerStatus::Online,
                last_active: spacetimedb_sdk::Timestamp::now(),
            };
            state_guard.update_player(player.clone());
            
            // Test retrieval
            let retrieved = state_guard.get_player(&Identity::default()).unwrap();
